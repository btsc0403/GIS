<template>
  ...
  <div v-if="showDetailModal" class="modal">
    <div class="modal-content">
      <button class="close-button" @click="showDetailModal = false">×</button>
      <h2>{{ selectedBuilding?.label }}</h2>
      <p>Latitude: {{ selectedBuilding?.lat }}</p>
      <p>Longitude: {{ selectedBuilding?.lng }}</p>
      <p>Description: {{ selectedBuildingDescription }}</p>
    </div>
  </div>
</template>
...
const highlightMarker = (subIndex: number) => {
  if (markers.value[subIndex]) {
    markers.value[subIndex].setIcon(
      L.icon({
        iconUrl: "/path/to/highlighted_marker_icon.ico",
        iconSize: [24, 32],
        iconAnchor: [12, 32]
      })
    )
...
.modal-content {
  ...
  width: 100%;
  text-align: center;
  position: relative; /* 需要确保它为相对定位，以便子元素能够使用绝对定位 */
}
...
.close-button:hover {
  color: #000; /* 悬停时的字体颜色 */
  text-shadow: 0 0 5px rgba(0, 0, 0, 0.5); /* 添加阴影效果 */
  background-color: transparent; /* 确保悬停时背景保持透明 */
  box-shadow: none; /* 确保悬停时没有任何阴影框 */
}
</style>
